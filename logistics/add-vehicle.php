...
// PHP form handling logic removed
?>
...
<body class="bg-gray-50">
    <div class="app-container" x-data="addVehicleForm()">
                    ...
                    <form @submit.prevent="submitForm" class="p-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
                            ...
                            <div>
                             ...
                             <div>
                                <label for="current_mileage" class="form-label">Current Mileage (if different from initial) </label>
                        ...
                        <div class="mt-8 flex justify-end gap-4">
                            ...
                            <button type="submit" class="btn btn-primary" :disabled="isLoading">
                                <span x-show="isLoading" class="animate-spin inline-block w-4 h-4 border-2 border-white border-t-transparent rounded-full mr-2"></span>
                                <span x-text="isLoading ? 'Saving...' : 'Add Vehicle'"></span>
                            </button>
                        </div>
                    </form>
                </div>
...
document.addEventListener('alpine:initializing', () => {
    Alpine.data('addVehicleForm', () => ({
        ...
        vehicleData: {
            ...
            purchase_date: '',
            initial_mileage: 0,
            current_mileage: 0, // Will default to initial_mileage if not set higher
            funded_by: '',
        ...
        formErrors: {},
        globalMessage: { text: '', type: '' }, // type: 'success' or 'error'
        apiBaseUrl: '/api/v1',
        ...
        init() {
            if (!this.token) {
             ...
             // Set current_mileage to initial_mileage if it's not already set or is less
            this.$watch('vehicleData.initial_mileage', (value) => {
                if (!this.vehicleData.current_mileage || this.vehicleData.current_mileage < value) {
                    this.vehicleData.current_mileage = value;
                }
            });
        },
        ...
        async submitForm() {
            this.isLoading = true;
            this.formErrors = {};
            this.setGlobalMessage('', ''); // Clear previous messages

            // Basic client-side check (can be expanded)
            if (!this.vehicleData.registration_no || !this.vehicleData.make || !this.vehicleData.model || !this.vehicleData.vehicle_type || !this.vehicleData.fuel_type || !this.vehicleData.chassis_no || !this.vehicleData.engine_no ) {
                this.setGlobalMessage('Please fill in all required fields marked with * (or ensure API provides specific errors).', 'error');
                 // Example: Highlight specific fields - this should ideally come from API validation for consistency
                if(!this.vehicleData.registration_no) this.formErrors.registration_no = "Registration number is required.";
                // ... add more for other required fields if you want client-side pre-checks
            ...
            try {
                ...
                if (!response.ok) {
                    ...
                    this.setGlobalMessage('Vehicle added successfully!', 'success');
                    // Optionally redirect or clear form
                    // window.location.href = 'vehicles.php';
                    this.resetForm(); // Clear form for another entry
                    setTimeout(() => { this.globalMessage.text = ''}, 3000); // Clear success message
                }
            } catch (error) {
        ...
        resetForm() {
            this.vehicleData = {
                registration_no: '', make: '', model: '', vehicle_type: '', capacity: '',
                fuel_type: '', chassis_no: '', engine_no: '', purchase_date: '',
                initial_mileage: 0, current_mileage: 0, funded_by: '', status: 'Available',
                insurance_expiry: '', road_license_expiry: '', next_service_due: '', notes: ''
            };
            this.formErrors = {};
        },
        ...
        handleUnauthorized() {
            ...
            this.token = null;
            // Consider redirecting to a login page
        }
    }));
});
</script>
